...
class TestEntranceViaRegFormEnterButton:
    def test_login_via_registration_form_enter_button(self, driver):
        # Arrange
        driver.get(Curls.auth_endpoint)
        driver.find_element(*Locators.LOGIN_BUTTON_VIA_REG_FORM).click()
        driver.find_element(*Locators.EMAIL).send_keys(*Credentials.email)
        driver.find_element(*Locators.PASSWORD).send_keys(*Credentials.password)
        # Act
        driver.find_element(*Locators.ENTER_BUTTON).click()
...
class TestEntranceViaPasswordResetPage:
    def test_enter_via_password_reset_page(self, driver):
        ...
        driver.find_element(*Locators.LOGIN_BUTTON_IN_PASSWORD_RESET_PAGE).click()
        driver.find_element(*Locators.EMAIL).send_keys(*Credentials.email)
        driver.find_element(*Locators.PASSWORD).send_keys(*Credentials.password)
        # Act
        driver.find_element(*Locators.ENTER_BUTTON).click()
